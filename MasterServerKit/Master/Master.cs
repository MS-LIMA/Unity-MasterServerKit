    ...
    public partial class MasterServerKit
    {
        public class Master
        {
            ...
            private static void Initialize()
            {
                ...
                MskDispatcher.DispatchRate = 10;

                MskDispatcher.Initialize();
                MskSpawner.Initialize();
                //MskMasterMono.Initialize();
                ...
                for (int i = 0; i < MskConfigMaster.MaxConnections; i++)
                {
            ...
            public static void StartMasterServer()
            {
                ...
                string host = MskConfigMaster.Ip;
                ushort port = MskConfigMaster.Port;

                IPAddress address = MskConfigMaster.IsIpDomain ? Dns.GetHostAddresses(host)[0] : IPAddress.Parse(host);
            ...
            public static void OnRoomRegisterRequested(MskSocket client, Packet packet)
            {
                ...
                MskSocket playerClient = MskSpawner.GetPlayerClientRequestedCreatingRoom(version, roomName);
